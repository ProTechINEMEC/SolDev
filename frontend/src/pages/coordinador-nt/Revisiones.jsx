...
function CoordinadorNTRevisiones() {
  ...
  const loadSolicitudes = async () => {
    ...
    try {
      const params = {
        ...
        estado: 'pendiente_revision_coordinador_nt',
  ...
  const columns = [
    {
      ...
      render: (codigo, record) => (
        <Link to={`/coordinador-nt/revision/${record.codigo}`}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <Title level={3} style={{ marginBottom: 0 }}>Revisiones Pendientes</Title>
          <Text type="secondary">Proyectos escalados por NT que requieren su revision</Text>
        </div>
      ...
      <Card>
        {loading ? (
          ...
          <Empty
            ...
            description="No hay solicitudes pendientes de revision"
          />
        ) : (
          <Table
...
export default CoordinadorNTRevisiones
