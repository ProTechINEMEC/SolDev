...
function GerenciaProyectoDetail() {
  ...
  const handleApproveReprogramacion = async () => {
    ...
    try {
      await proyectosApi.aprobarReprogramacionGerencia(codigo, reprogramacion.id, {
        accion: 'aprobar',
        comentario: rescheduleApproveComentario || null,
        fecha_inicio: rescheduleDates?.fecha_inicio,
        fecha_fin: rescheduleDates?.fecha_fin
      })
      message.success('Reprogramación aprobada — fechas del proyecto actualizadas')
  ...
  const handleCancelar = async () => {
    try {
      ...
      setActionLoading(true)
      await proyectosApi.cancelarGerencia(codigo, { motivo: values.motivo })
    ...
    <div style={{ padding: 24 }}>
      ...
      <Card>
            ...
            <Alert type="warning" showIcon
              ...
              description={
                <Space direction="vertical" size={4}>
                  ...
                  {reprogramacion.coordinador_nombre && (
                    <Text type="success">Aprobado por Coord. NT: {reprogramacion.coordinador_nombre} — Fechas: {dayjs(reprogramacion.fecha_inicio_coordinador).format('DD/MM/YYYY')} - {dayjs(reprogramacion.fecha_fin_coordinador).format('DD/MM/YYYY')}</Text>
                  )}
                  {reprogramacion.comentario_coordinador && (
                    <Text>Comentario Coord.: {reprogramacion.comentario_coordinador}</Text>
                  )}
                </Space>
              }
        ...
        {reprogramacion?.estado === 'pendiente_coordinador' && (
          <Alert style={{ marginBottom: 16 }} type="info" showIcon
            message="Reprogramación solicitada, pendiente de Coordinador NT"
          />
        )}
      ...
      <Modal title="Cancelar Proyecto" open={cancelModalVisible} onOk={handleCancelar}
        ...
        <Alert type="error" message="Esta accion no se puede deshacer"
          description="El proyecto sera marcado como cancelado por Gerencia y no podra continuarse."
          style={{ marginBottom: 16 }} />
      ...
      {/* Reschedule Calendar (Gerencia picks date to approve) */}
      <SchedulingCalendar
      ...
      <Modal
        ...
        okText="Aprobar y Reprogramar"
        cancelText="Cancelar"
      >
        <Alert type="info" message="Al aprobar, todas las tareas del proyecto serán recalculadas con las nuevas fechas." style={{ marginBottom: 16 }} />
        {rescheduleDates && (
...
export default GerenciaProyectoDetail
